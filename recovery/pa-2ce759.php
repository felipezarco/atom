			...
			<div class="container-fluid">
					<div id="navigation">
						<div class="table-responsive">
							<table class="table mb-0" id="my-table" style="min-width: 600px;margin: 0 auto;max-width: 1010px;" >
								<thead>
									<tr></tr>
								</thead>
								<tbody>
									...
									<td style="text-align: right;">
										<ul class="list-unstyled topbar-nav mb-0" id="tabulacao">
											<li class="hidden-sm">
												...
												<div class="dropdown-menu">
												<?php
												$sqltab =  mysql_query("SELECT * FROM TABULACAO_CAD WHERE AGENCIA ='$idagencia' ")or die(mysql_error());
		...
		<div class="modal-content" id="modal-dados-cliente">
			...
			<div class="modal-body">
				<div class="table-responsive" id="tabela-dados-cliente">
					<table class="table mb-0" id="my-table" style="margin-bottom: 10px !important;">
						<thead>
							<tr>
				  ...
				  <div class="modal-body">
					<div class="table-responsive">
							...
							<thead>
								...
								<tbody id="1" class="matriculastabela">
								</tbody>
							</table>
							<table class="table mb-0" id="my-table">
								<thead>
								...
								<tbody id="1" class="matbancotabela">
								</tbody>
							</table>
						</div>
					</div>
...
// var ramal_ativo = <?php echo $ramal_ativo ?>;
// var operador = <?php echo $operador ?>;
// $(window).on( "unload", function () {
//    $.ajax({
// 		 type: 'POST',
// 		 data: { ramal_ativo: 2100, operador: <?php echo $operador ?> },
// 		 async: false,
//      url: '/functions/sairpa.php'
//    });
// 	 // return "disconnected";
// });
//
...
// 		if (e.key=="F5") { window.onbeforeunload = function ConfirmLeave() { return ""; }; }
// 		else if (e.key.toUpperCase() == "W" && prevKey == "CONTROL") { window.onbeforeunload = function ConfirmLeave() { return ""; }; }
// 		else if (e.key.toUpperCase() == "R" && prevKey == "CONTROL") { window.onbeforeunload = function ConfirmLeave() { return ""; }; }
// 		else if (e.key.toUpperCase() == "F4" && (prevKey == "ALT" || prevKey == "CONTROL")) { window.onbeforeunload = function ConfirmLeave() { return ""; }; }
...
function validarElemento(id, tipo, valido) {
	...
	// 	<i class="validar-endereco-`+idend+` `+isMarkedClass(valido)+`" style="font-size: 19px;"></i>
	// </span>
	...
	$.ajax({
		...
		success: (data) => {
			...
			let newValid = valido ? 0 : 1;
			 ...
			 // Cascade update exceptions

			 // Telefone --> TelefoneDiscado
			 if(tipo == 'telefone')
			 {
...
function dadosCliente(data) {
	...
	for (i = 0; i < endObj["lista"].length; i++)
	{
		...
		let valido = parseInt(endObj["lista"][i]["CLI_END_VALIDO"]) ;
	...
	if(destino) { $("#td-telativo").append(`<div id="telativo" style="float: right; margin-left: 5px; margin-top: 5px;">`+destino+`</div>`); }

	for (var z = 0; z < array_telefones.length; z++) {
		let telefone = parseInt(array_telefones[z][1]);
		let telefoneDiscado = parseInt(destino);

		// console.log(telefone+"="+telefoneDiscado);
	...
	for (k = 0; k < emailObj["lista"].length; k++)
	{
		...
		// console.log('VALIDO: ',email_valido);
	...
	$(".matbancotabela").html("");
	for (l = 0; l < matriculaObj["lista"].length; l++)
	{
		var idben 			   			= matriculaObj["lista"][l]["CLI_BEN_ID"] || vazio;
		var ben_nb 		       		= matriculaObj["lista"][l]["CLI_BEN_NB"] || vazio;
		var ben_dib  	        	= matriculaObj["lista"][l]["CLI_BEN_DIB"];
		var ben_salario         = matriculaObj["lista"][l]["CLI_BEN_SALARIO"] || vazio;
		var ben_especie 	   		= matriculaObj["lista"][l]["CLI_BEN_ESPECIE"] || vazio;
		var ben_margem  	    	= matriculaObj["lista"][l]["CLI_BEN_MARGEM"] || vazio;
		var ben_banco_recebe    = matriculaObj["lista"][l]["CLI_BEN_BANCO_RECEBE"] || vazio;
		var ben_agencia_recebe 	= matriculaObj["lista"][l]["CLI_BEN_AGENCIA_RECEBE"] || vazio;
		var ben_conta_recebe 		= matriculaObj["lista"][l]["CLI_BEN_CONTA_RECEBE"] || vazio;
	...
	for (m = 0; m < bancoObj["lista"].length; m++)
	{
		...
		if (grupoproduto == novo)
		{
			...
			$("#tab_novo_td").append(`<td class="thtable" id="calculo-banco-`+grupoidbanco+`" data-calculo="`+grupoidbanco+`"><a id="recalc-banco-`+grupoidbanco+`" href="CadNovo/`+idcli+`/`+novo+`/`+grupoidbanco+`/`+(calculonovo+"").replace(".",",")+`" class="cal_novo btn btn-soft-info tdbutton">`+formatter.format(calculonovo)+`</a></td>`);
		}
...
$(function(){
  console.log("ID do Cliente: "+idcli);
